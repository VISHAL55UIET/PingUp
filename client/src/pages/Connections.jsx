...
import { Users, UserPlus, UserCheck, UserRoundPen, MessageSquare } from 'lucide-react' // Removed unused 'icons'
import { useNavigate } from 'react-router-dom'; // Removed unused 'data' import
import {
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {dataArray.map((tab) => (
            <button 
              ...
              key={tab.label} 
              className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-slate-100 font-medium text-black' : 'text-gray-500 hover:text-black'}`}
            >
              <tab.icon className='w-4 h-4' />
              <span className='ml-1'>{tab.label}</span>
              {/* Added logic to show count in tab pill if needed, using item.value.length */}
          ...
          {dataArray.find((item) => item.label === currentTab).value.map((user) => (
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div className='flex-1'>
                ...
                {/* Fixed: "gray-590" is not a valid Tailwind class, changed to "gray-500" */}
                <p className='text-sm text-gray-500'>{user.bio.slice(0, 30)}...</p>
                
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  {/* Fixed: "texdt-sm" to "text-sm" and "transitiion" to "transition" */}
                  ...
                  {currentTab === 'Following' && (
                    /* Fixed: "Unfallow" to "Unfollow" and "active-95" to "active:scale-95" */
